...
from sentinel_iron.brokers.optimus.config import BrokerEnvironment, OptimusConfig, OptimusRoute
from sentinel_iron.domain.enums import OrderSide, OrderType
from sentinel_iron.domain.orders import BrokerOrder
from sentinel_iron.ports.broker import BrokerCancellationError, BrokerConnectionError, BrokerSubmissionError
from sentinel_iron.ports.market_data import MarketDataError
...
class FailingTransport(RecordingTransport):
    ...
    def request(
        ...
        from sentinel_iron.brokers.optimus.adapter import OptimusBridgeError
...
def _headers() -> dict[str, str]:
    return {
        ...
        "X-Optimus-App-Name": "sentinel-iron-test",
...
def test_optimus_adapter_fails_closed_for_historical_daily_bars():
    transport = RecordingTransport(())
    broker = _adapter(transport)
