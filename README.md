...
```powershell
cd C:\Users\automation\GitBots\Sentinel-Iron
...
`sentinel_iron.brokers.ibkr.IbkrBroker` implements the broker port around a TWS or IB Gateway client contract. It maps IBKR account summary rows, position rows, next-valid order IDs, futures contracts, order payloads, cancel requests, what-if margin previews, and historical daily bars into shared broker and market-data ports. `sentinel_iron.brokers.ibkr.IbapiTwsClient` provides the concrete optional `ibapi` transport for TWS or IB Gateway callback flows, including `reqHistoricalData` daily bars.
...
`sentinel_iron.brokers.tradestation.TradeStationBroker` implements the broker port with bearer-token HTTP calls against the configured paper or live TradeStation base URL. It validates the configured account, fetches balances and positions, submits approved orders, requests order cancellation, and fetches normalized daily historical bars through TradeStation barcharts using the same application-layer risk, readiness, submission, cancellation, reconciliation, market-data, and audit services used by every broker.
...
```powershell
sentinel-iron config-check
sentinel-iron config-check --broker ibkr
sentinel-iron config-check --broker tradestation
sentinel-iron config-check --broker ninjatrader
sentinel-iron config-check --broker optimus
```
...
sentinel-iron broker-connect --broker tradestation --audit-log data/audit.jsonl
sentinel-iron broker-connect --broker ibkr --audit-log data/audit.jsonl
sentinel-iron broker-connect --broker ninjatrader --audit-log data/audit.jsonl
sentinel-iron broker-connect --broker optimus --audit-log data/audit.jsonl
```
...
```powershell
sentinel-iron kill-switch --state-file data/kill_switch.json status
sentinel-iron kill-switch --state-file data/kill_switch.json --audit-log data/audit.jsonl activate --reason "operator halt before news release"
sentinel-iron kill-switch --state-file data/kill_switch.json --audit-log data/audit.jsonl clear
```
...
```powershell
sentinel-iron reconcile --broker tradestation --internal-positions data/internal_positions.json --audit-log data/audit.jsonl
sentinel-iron reconcile --broker ibkr --internal-positions data/internal_positions.json --audit-log data/audit.jsonl
sentinel-iron reconcile --broker ninjatrader --internal-positions data/internal_positions.json --audit-log data/audit.jsonl
sentinel-iron reconcile --broker optimus --internal-positions data/internal_positions.json --audit-log data/audit.jsonl
```
...
```powershell
sentinel-iron margin-schedules --schedule-file data/margin_schedules.json validate
```
...
```powershell
sentinel-iron instrument-catalog --catalog-file data/instruments.json validate
sentinel-iron instrument-catalog --catalog-file data/instruments.json validate --trading-day 2026-09-14
```
...
```powershell
sentinel-iron flatten --broker tradestation --audit-log data/audit.jsonl --confirm FLATTEN-LIVE-POSITIONS
sentinel-iron flatten --broker ibkr --audit-log data/audit.jsonl --confirm FLATTEN-LIVE-POSITIONS
sentinel-iron flatten --broker ninjatrader --audit-log data/audit.jsonl --confirm FLATTEN-LIVE-POSITIONS
sentinel-iron flatten --broker optimus --audit-log data/audit.jsonl --confirm FLATTEN-LIVE-POSITIONS
```

When `BROKER_ENV=live`, add the live activation token in the same invocation:

```powershell
sentinel-iron flatten --broker tradestation --audit-log data/audit.jsonl --confirm FLATTEN-LIVE-POSITIONS --live-trading-activation ENABLE-LIVE-TRADING
```
...
No strategy can bypass broker ports, reconciliation, risk checks, or audit logging. The first strategy target is diversified futures trend following with volatility targeting after live broker lifecycle handling is in place. `sentinel_iron.strategies.trend_following.calculate_volatility_adjusted_trend_signal` can convert multi-lookback returns into volatility-normalized scores before they enter portfolio sizing, so weak and strong trends are not treated identically. `sentinel_iron.strategies.composite.combine_weighted_signals` can blend trend and carry scores per market, and `sentinel_iron.application.strategy_targets.build_strategy_position_targets` can turn those signals into delivery-date-aware, gross-risk-capped target positions before order planning creates broker intents.
